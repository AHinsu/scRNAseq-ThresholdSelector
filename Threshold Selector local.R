  ...
  sidebarLayout(
    sidebarPanel(
      radioButtons("data_source", "Choose Data Source:",
                   choices = c("Select from Environment", "Upload CSV", "Upload RDS")),
      
      conditionalPanel(
        condition = "input.data_source == 'Select from Environment'",
        uiOutput("env_df_selector")
      ),
      ...
      tags$div(
        ...
        box(title = "nCount_RNA", status = "primary", solidHeader = TRUE, width = 12,
            fluidRow(
        ...
        box(title = "nFeature_RNA", status = "primary", solidHeader = TRUE, width = 12,
            fluidRow(
        ...
        box(title = "percent.mt", status = "primary", solidHeader = TRUE, width = 12,
            fluidRow(
...
server <- function(input, output, session) {
  ...
  output$env_df_selector <- renderUI({
    if (exists(".GlobalEnv")) {
      selectInput("env_df", "Select Dataframe:", choices = ls(envir = .GlobalEnv))
    } else {
      helpText("Global environment not available in browser mode.")
    }
  })
  
  selected_df <- reactive({
    if (input$data_source == "Select from Environment" && exists(".GlobalEnv")) {
  ...
  output$filtered_counts <- renderTable({
    ...
    df_removed <- df_all %>%
      filter(!cell_id %in% df_filtered$cell_id)
